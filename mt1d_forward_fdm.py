...
class MT1DForwardFDM_TM:
    def __init__(self, rho, h, t_sample=71, dz=20.0):
        ...
        self.ph = np.zeros(t_sample)
        # 计算结果存储
        self.Z_TM_surface = np.zeros(t_sample, dtype=np.complex128)
        self.Hx_TM_surface = np.zeros(t_sample, dtype=np.complex128)
        self.Ey_TM_surface = np.zeros(t_sample, dtype=np.complex128)
        self._run_forward()
    ...
    def _solve_frequency(self, tt):
        ...
        self.Z_TM_surface[tt] = Z_TM_g
        self.pc[tt] = np.abs(Z_TM_g)**2 / (mu * Omega)
        self.ph[tt] = -np.angle(Z_TM_g, deg=True)
